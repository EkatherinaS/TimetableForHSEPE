...
public class DemoActivity extends AppCompatActivity {
    ...
    private void clickButton() {
        ...
        if((numberVal.toString()).length() > 5) {
            Toast.makeText(DemoActivity.this, "Число не должно превышать 10000", Toast.LENGTH_LONG).show();
        }
        else {
    ...
    private void clickButton2() {
        ...
        if((numberVal.toString()).length() > 2) {
            Toast.makeText(DemoActivity.this, "Число не должно превышать 19", Toast.LENGTH_LONG).show();
        }
        else {
            int count = Integer.parseInt(numberVal);
            if (count > 19) {
                ...
                result.setText(String. format("Result: %d", mul));
            }
        }
    }
